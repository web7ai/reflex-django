...
from reflex_django.project import find_manage_py, rxconfig_path
...
def _should_use_rxconfig_file() -> bool:
    ...
    explicit = getattr(settings, "REFLEX_DJANGO_USE_RXCONFIG_FILE", None)
...
def load_rxconfig_file_if_present() -> Config | None:
    ...
    if rxconfig_path() is None:
        return None
    from reflex_django.integration import call_original_get_config

    return call_original_get_config()


def build_rx_config_from_django() -> Config:
    """Synthesize ``rx.Config`` from :func:`reflex_django.urls.reflex_mount` registration."""
...
def _merge_mount_plugins(config: Config) -> Config:
    """Apply plugins registered via :func:`reflex_django.urls.reflex_mount`."""
...
def install_rxconfig_module(config: Config) -> None:
    ...
    from reflex_django.project import RXCONFIG_SYNTHETIC_ATTR
...
def build_merged_config_for_django_mode() -> Config:
    ...
    ``REFLEX_DJANGO_MATERIALIZE_RXCONFIG`` / ``REFLEX_DJANGO_USE_RXCONFIG_FILE`` are enabled.
    """
    settings = _django_settings()
    if getattr(settings, "REFLEX_DJANGO_MATERIALIZE_RXCONFIG", False):
