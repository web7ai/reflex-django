...
def _settings_include_groups() -> bool:
    ...
    return bool(getattr(settings, "RX_USER_SNAPSHOT_INCLUDE_GROUPS", False))
...
def _collect_middleware_mirror_snapshot() -> dict[str, Any]:
    ...
    if getattr(settings, "RX_MIRROR_MESSAGES", True):
        try:
    ...
    if getattr(settings, "RX_MIRROR_CSRF", True):
        try:
    ...
    if getattr(settings, "RX_MIRROR_LANGUAGE", True):
        try:
        ...
        *,
        ...
        Args:
            include_groups: When ``None``, use
                ``settings.RX_USER_SNAPSHOT_INCLUDE_GROUPS``.
