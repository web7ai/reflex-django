...
In :class:`~reflex_django.setup.routing.UrlRoutingMode.DJANGO_OUTER` mode, the
ASGI process boots from
:func:`reflex_django.asgi.entry.application`, which composes:
...
logger = logging.getLogger("reflex_django.asgi.django_outer")
...
class DjangoOuterDispatcher:
    ...
    async def _serve_compile_dev_build_id(send: ASGISend) -> None:
        ...
        from reflex_django.dev.runners.frontend import build_id_for_disk_bundle
