...
def patch_vite_config(
    ...
    *,
    ...
    content = inject_vite_dev_proxy(content, routes=coerced)
...
def resolve_vite_dev_proxy_routes() -> tuple[ViteProxyRoute, ...]:
    ...
    from reflex_django.django_http_subprocess import resolve_django_http_upstream
...
def ensure_vite_django_dev_proxy(
    ...
    *,
    ...
    if not _vite_proxy_is_complete(content, routes=coerced):
        updated = patch_vite_config(content, routes=coerced)
        if updated != content:
...
def ensure_vite_django_dev_proxy_from_config() -> bool:
    ...
    Two-port dev (``REFLEX_DJANGO_SEPARATE_DEV_PORTS=1``) serves the SPA from
