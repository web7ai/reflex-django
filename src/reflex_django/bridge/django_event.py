...
- It synthesizes a :class:`django.http.HttpRequest` from
  ...
  :mod:`reflex_django.bridge.context` contextvar (see :func:`begin_event_request`).
- Handlers can then call :func:`reflex_django.current_user` etc. without
  needing direct access to the event payload.
- When ``USE_I18N`` and ``RX_I18N_EVENT_BRIDGE`` are true, it also
...
def _populate_post_from_payload(
    ...
    ``RX_EVENT_POST_FROM_PAYLOAD = True`` so middleware that
...
class DjangoEventBridge(Middleware):
    ...
    async def preprocess(
        ...
        2. Run ``settings.MIDDLEWARE`` (filtered by
           ``RX_EVENT_MIDDLEWARE_SKIP``) against it through
           :class:`reflex_django.bridge.event_handler.EventMiddlewareHandler`. This is
        ...
        4. If the middleware chain short-circuited with a 3xx redirect and
           ``RX_AUTO_REDIRECT_FROM_MIDDLEWARE`` is enabled,
