...
def _parse_argv(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="reflex_django._backend_runner")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from reflex_django.dev_watch import BACKEND_RELOAD_DELAY_S, backend_reload_excludes

    reload = bool(args.reload) and _backend_reload_enabled()
    run_kwargs: dict[str, Any] = {
        "app": "reflex_django.asgi_entry:application",
        "factory": False,
        "host": args.host,
        "port": int(args.port),
