...
- The behaviour is gated by ``settings.RX_RENDER_SPA_VIA_TEMPLATE_ENGINE``
  (default ``True``).
"""
...
logger = logging.getLogger("reflex_django.mount.spa_template")
...
def _render_via_template_engine_enabled() -> bool:
    ...
    try:
        ...
        return bool(
            getattr(settings, "RX_RENDER_SPA_VIA_TEMPLATE_ENGINE", True),
        )
    except Exception:
        return True
...
def compile_dev_reload_script(*, wait_for_ready: bool = False) -> str:
    ...
    from reflex_django.dev.runners.frontend import BUILD_ID_PATH
