...
When ``RX_DEV_PROXY=1`` and ``RX_SEPARATE_DEV_PORTS=0``,
...
Default two-port dev uses ``reflex run`` (browse ``:3000``
...
logger = logging.getLogger("reflex_django.dev.proxy")

_DEV_PROXY_ENV = "RX_DEV_PROXY"
...
def dev_uses_separate_ports() -> bool:
    ...
    """
    env = os.environ.get("RX_SEPARATE_DEV_PORTS")
    if env is not None:
        return str(env).strip().lower() not in {"0", "false", "no"}
    try:
        from django.conf import settings

        return bool(getattr(settings, "RX_SEPARATE_DEV_PORTS", False))
    except Exception:
        return False


def dev_proxy_explicitly_enabled() -> bool:
    """Return True only when ``RX_DEV_PROXY`` is explicitly truthy.

    ``reflex run`` may set this in the Vite loop so the startup probe
...
def _resolve_frontend_port_from_config() -> int | None:
    ...
    try:
        ...
        setting_port = getattr(settings, "RX_FRONTEND_PORT", None)
...
def _resolve_backend_port_from_config() -> int | None:
    ...
    env_port = os.environ.get("RX_BACKEND_PORT")
    if env_port and env_port.isdigit():
        return int(env_port)
    try:
        from django.conf import settings

        setting_port = getattr(settings, "RX_BACKEND_PORT", None)
        if isinstance(setting_port, int) and setting_port > 0:
            return setting_port
    except Exception:
        pass
...
def _dev_vite_target_or_none() -> str | None:
    ...
    if not getattr(settings, "DEBUG", False):
        return None
    if str(os.environ.get("RX_DEV_PROXY", "")).strip().lower() in {
        "0",
        "false",
        "no",
    }:
        return None
    # ``RX_DEV_PROXY=1`` sets the env explicitly; that must win over
    # a project setting such as ``RX_DEV_PROXY = False`` in dev.py.
    if not dev_proxy_explicitly_enabled() and not getattr(
        settings, "RX_DEV_PROXY", True
    ):
        return None
    ...
    if port is None:
        return None
    host = os.environ.get("RX_FRONTEND_HOST", "127.0.0.1")
...
async def reverse_proxy_to_vite(
    ...
    try:
        ...
        msg = (
            "reflex_django.dev.proxy requires httpx for the dev Vite reverse-proxy. "
            "Install with `pip install httpx` or set RX_DEV_PROXY=0 and "
    ...
    # a streaming body) so :mod:`reflex_django.mount.spa_template` can pipe it
...
async def proxy_websocket_to_vite(scope: Any, receive: Any, send: Any) -> None:
    ...
    separately by :class:`~reflex_django.asgi.django_outer.DjangoOuterDispatcher`
