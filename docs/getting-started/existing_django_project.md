...
Auto-detection reads the **first segment** of each top-level `path()` (`path("api/", include(...))` covers `/api/products/`, and so on). Pass explicit `REFLEX_DJANGO_DJANGO_PREFIX` only if auto-detection misses a prefix. See [The three knobs](../overview/concepts.md).
...
If you need Django on `runserver` separately from Reflex, set `RXDJANGO_PROXY_SERVER` and run both processes. See [Routing](../internals/routing.md) and [Local development](local_development.md).
