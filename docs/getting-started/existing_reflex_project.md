...
| Custom Reflex plugins | Move plugins to `RX_PLUGINS` |
...
**After:** `config/settings.py` with `RX_CONFIG`, pages in `myshop/views.py`, `python manage.py run_reflex`.
...
RX_PLUGINS = ["reflex.plugins.RadixThemesPlugin"]
```

Delete `rxconfig.py`, or set `RX_USE_RXCONFIG_FILE = True` only if CI still needs the file temporarily.

Remove any legacy Django integration plugin from old `rxconfig.py` plugin lists. Django bootstrap is automatic from settings in v3.
...
For a `pages/` package, set `RX_PAGE_PACKAGES` or import submodules from `views.py`. See [Pages in views.py](../guides/pages.md).
...
Use `AppState` instead of `rx.State` when you need `self.request.user`, sessions, or the ORM. See [AppState](../guides/state.md).
...
Set `RX_PROXY_SERVER` when Django should run on `runserver` separately from Reflex. See [Local development](local_development.md).
...
- **Plugins missing**: move them to `RX_PLUGINS` in settings.
