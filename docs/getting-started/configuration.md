...
| `django_plugin` | `{}` | Extra kwargs for the built-in Django integration plugin. Merged with `RX_PLUGIN`. |
...
| Setting | Default | What it does |
|:---|:---|:---|
| `RX_PAGE_PACKAGES` | `[]` | Explicit list of page modules. If non-empty, disables auto-discovery. |
| `RX_AUTO_DISCOVER_PAGES` | `True` (deprecated) | Walk `INSTALLED_APPS` and import `{app}.views` at compile time. |
| `RX_PAGE_APPS` | `None` | Allowlist of app labels to scan. `None` means all of them. |
| `RX_PAGE_MODULE` | `"views"` | Which submodule to import per app. |
...
| `RX_PLUGIN` | `{}` | Extra kwargs for the built-in Django integration plugin. |
| `RX_AUTO_PLUGIN` | `True` | Always enabled. Kept for backwards compatibility. |

### Auth

| Setting | Default | What it does |
|:---|:---|:---|
| `RX_LOGIN_URL` | `"/login"` | Where `@login_required` redirects to. |
| `RX_AUTH` | see [Login and sessions](../guides/authentication.md) | Built-in auth pages and branding. |
...
| `RX_PLUGIN` | Legacy plugin kwargs (mostly unused; integration is automatic) |
