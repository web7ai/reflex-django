...
| `REFLEX_DJANGO_AUTH` | see [Login and sessions](../guides/authentication.md) | Built-in auth pages and branding. |
...
Set `RXDJANGO_PROXY_SERVER` and run Django with `runserver` in a second terminal when you need Django isolated from the Reflex backend. See [Routing](../internals/routing.md).
