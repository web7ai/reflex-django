...
When `RX_PROXY_SERVER` is unset, `get_or_create_app()` attaches `make_dispatcher()` to the Reflex app's `api_transformer`. Requests to `/admin`, `/api`, and other configured prefixes hit Django's URLconf in the **same process** as the Reflex backend.
