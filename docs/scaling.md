...
REFLEX_DJANGO_EVENT_CACHE = "reflex_events"
...
# REFLEX_DJANGO_EVENT_METRICS = True
```

See [Settings reference](settings_reference.md) for every knob and default.
...
| Button clicks, form edits | Reflex `/_event` | `REFLEX_DJANGO_EVENT_BRIDGE_MODE`, per-State override |
| File uploads | Reflex `/_upload` | Always at least `auth_only` tier (session + auth) |
| UI-only state (theme, modals) | Reflex `/_event` | `_reflex_django_bridge = "none"` or `"smart"` mode |

Django developers configure; reflex-django executes. No hidden env vars unless documented as overrides (for example `RXDJANGO_PROXY_SERVER` for split-process dev).
...
!!! warning "Use `_reflex_django_bridge`, not `reflex_django_bridge`"
...
| `auth_only` | `REFLEX_DJANGO_AUTH_ONLY_MIDDLEWARE` | Only for `DjangoUserState` handlers |
| `none` | Skipped | No |

### Project default

```python
REFLEX_DJANGO_EVENT_BRIDGE_MODE = "full"   # legacy default
REFLEX_DJANGO_EVENT_BRIDGE_MODE = "smart"  # plain rx.State -> none; AppState -> full
REFLEX_DJANGO_EVENT_BRIDGE_MODE = "none"   # skip chain unless overridden
```
...
REFLEX_DJANGO_EVENT_CACHE_TTL = 60           # 0 = disabled
...
When `REFLEX_DJANGO_PERFORMANCE_PRESET = "lean"` and mirror settings still match library defaults, reflex-django disables:
...
When `RXDJANGO_PROXY_SERVER` points at a separate `runserver`, reflex-django skips `make_dispatcher` on the Reflex backend (saves memory). Production still uses your edge proxy; this is a dev layout only. See [Local development](local_development.md).
...
| Need | Setting / hook |
|:---|:---|
| Project-wide bridge behavior | `REFLEX_DJANGO_EVENT_BRIDGE_MODE` |
| Full custom tier logic | `REFLEX_DJANGO_EVENT_BRIDGE_RESOLVER` |
| One hot state class | `_reflex_django_bridge = "none"` |
| Skip heavy middleware on events | `REFLEX_DJANGO_EVENT_MIDDLEWARE_SKIP` |
| Auth-only middleware list | `REFLEX_DJANGO_AUTH_ONLY_MIDDLEWARE` |
| Disable middleware entirely | `REFLEX_DJANGO_RUN_MIDDLEWARE_CHAIN = False` |
| Smaller WebSocket deltas | `REFLEX_DJANGO_PERFORMANCE_PRESET = "lean"` |
| Session cache between events | `REFLEX_DJANGO_EVENT_CACHE` + `CACHES` + TTL |
| Flush cache on logout | `invalidate_event_cache()` |
| Multi-worker Reflex | `REFLEX_DJANGO_RX_CONFIG["redis_url"]` |
| Skip URL resolve on events | `REFLEX_DJANGO_EVENT_RESOLVE_URL = False` |
| Debug timings | `REFLEX_DJANGO_EVENT_METRICS = True` |
...
[Deployment ->](deployment.md) for build and proxy detail, or [Custom middleware in events ->](django_middleware_to_reflex.md) for tier internals.