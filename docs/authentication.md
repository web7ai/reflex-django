...
See [How they fit together](how_they_fit.md) and [WebSocket event pipeline](websocket_event_pipeline.md) for the full picture.
...
If the user is not authenticated (or lacks permission), the handler does not run. The decorator returns `rx.redirect(...)` to `REFLEX_DJANGO_LOGIN_URL` (default `/login`).
...
When `REFLEX_DJANGO_AUTH["ENABLED"]` is true (default), auth pages register automatically during page preparation (import/compile time). No `views.py` boilerplate required.
...
Customize the canned login, register, and password-reset pages from `REFLEX_DJANGO_AUTH` in `settings.py`. No subclasses required for text and logo changes.
...
If your app uses `AppState` everywhere, `MyState.is_authenticated` is usually enough. When `REFLEX_DJANGO_AUTH_AUTO_SYNC = True` (default), snapshot fields refresh on every event.
...
| Brand built-in auth pages | `REFLEX_DJANGO_AUTH`, see [Make it yours](#make-it-yours) |
...
You learned that Django session auth flows through the event bridge into `self.request.user`, that snapshots are for UI only, and that built-in auth pages register at compile time and customize through `REFLEX_DJANGO_AUTH`.

---

**Next up:** [Media files](media_files.md)