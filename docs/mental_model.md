...
Starts **two** dev servers (Vite `:3000` + backend `:8000`). Open **http://localhost:3000/** for the SPA. Admin and API live on `:8000` and the SPA reaches them via `env.json`. Optional: `--env dev` for compile-only dev on `:8000`. See [Local development](local_development.md).
...
See [Configuration](configuration.md) and [Settings reference](settings_reference.md) for every knob.
...
See [Pages in views.py](pages_in_views.md) for layout helpers, auth pages, and troubleshooting.
...
**Next up:** [Why reflex-django exists →](why_reflex_django.md)
