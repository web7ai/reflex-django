...
Use Django's translation stack inside Reflex. When bridge is enabled and an event resolves to a Django-bound tier, <code>LocaleMiddleware</code> runs on the synthetic request, so <code>gettext</code> and <code>translation.get_language()</code> work in handlers the same way they do in Django views.
</p>
