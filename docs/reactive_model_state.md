...
- You want less boilerplate than the manual approach in [CRUD the manual way](crud_without_mixins.md).
...
`UserScopedMixin` filters `get_queryset`, `get_object_lookup`, and `get_create_kwargs` by the logged-in user. See [Mixins](reflex_django_mixins.md) for manual composition.
...
For those, stay with plain `AppState` as in [CRUD the manual way](crud_without_mixins.md). If you need an explicit serializer class or named handlers like `save_post`, see [ModelCRUDView](crud_with_mixins_and_states.md).
...
**Next up:** [ModelCRUDView with serializers →](crud_with_mixins_and_states.md)