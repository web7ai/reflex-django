...
!!! tip "Prefer less code later?"
    After this page, [CRUD with ModelState](../guides/crud.md#modelstate) covers the same feature in a fraction of the lines.
...
`await ProductSerializer(qs, many=True).adata()` turns a queryset into a JSON-safe list of dicts. More detail in [Model serializers](../guides/serializers.md).
...
**Next up:** [CRUD with ModelState →](../guides/crud.md#modelstate)