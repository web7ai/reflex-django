...
Because `CatalogState` inherits from `AppState`, `self.request.user` is available for scoped queries. See [AppState bridge](state_management.md).
...
For many-to-many or reverse foreign keys, use `prefetch_related`. With [`ModelCRUDView`](crud_with_mixins_and_states.md), declare these in `Meta`:
...
`ModelState` is an `AppState`, so you keep `self.request.user`. See [Reactive model state](reactive_model_state.md).
