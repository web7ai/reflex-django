...
Open **`http://localhost:3000/`** for UI work. Admin and API work at **`http://localhost:3000/admin/`** (proxied) or **`http://localhost:8000/admin/`** (direct to Reflex backend). Set **`RX_PROXY_SERVER`** only when Django runs on a separate server  -  see [Routing](../internals/routing.md).
...
| `--env dev` | Compile dev on one port (`RX_COMPILE_DEV=1`). Pass `--with-vite` to add HMR on `:3000` again. |
...
2. Sets two-port dev (RX_SEPARATE_DEV_PORTS=1, RX_DEV_PROXY=0)
3. Patches .web/vite.config.js (Django + Reflex → Reflex backend, or split if RX_PROXY_SERVER set)
...
```text
1. install_reflex_django_integration()
2. export_reflex (unless --skip-rebuild); sets RX_DEBUG=0
...
By default, `run_reflex` watches `BASE_DIR/**/*.py`. Built-in exclusions: `.web/`, `.venv/`, migrations, tests. There are no `RX_WATCH_PATHS` settings.
...
| CSRF on admin in dev | [Local development](../getting-started/local_development.md) |
