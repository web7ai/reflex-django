...
Open **`http://localhost:3000/`** for UI work. Admin and API work at **`http://localhost:3000/admin/`** (proxied) or **`http://localhost:8000/admin/`** (direct to Reflex backend). Set **`RXDJANGO_PROXY_SERVER`** only when Django runs on a separate server  -  see [Routing](../internals/routing.md).
