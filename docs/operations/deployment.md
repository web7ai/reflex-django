...
Set `RXDJANGO_PROXY_SERVER` when Django runs on `runserver` and Reflex runs via `run_reflex`. This is a **dev-only** layout; production still uses Django ASGI plus a separate Reflex backend behind your edge proxy. See [Routing](../internals/routing.md).
...
Full example blocks and Caddy config are in the repo docs history; see also [Media files](../guides/media.md) for `/media/`.
