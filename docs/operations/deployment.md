...
`run_reflex --env prod` also sets `REFLEX_DJANGO_DEBUG=0`. If your settings derive `DEBUG` from that env var, ensure production settings still set `DEBUG = False` explicitly.
...
- Multi-replica deployments need **Redis** (`REFLEX_DJANGO_RX_CONFIG["redis_url"]`) and **sticky sessions** at the load balancer.
...
Set `RXDJANGO_PROXY_SERVER` when Django runs on `runserver` and Reflex runs via `run_reflex`. This is a **dev-only** layout. See [Routing](../internals/routing.md).
...
With `DEBUG = False`, the dev proxy is off and the SPA serves from disk. You can also set `REFLEX_DJANGO_DEV_PROXY=0` explicitly. See [Settings reference](../reference/settings.md).
...
- **Redis:** required for multi-worker or multi-replica Reflex via `REFLEX_DJANGO_RX_CONFIG["redis_url"]`.
