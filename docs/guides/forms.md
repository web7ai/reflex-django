...
Built-in login and register pages (via `add_auth_pages()`) use `RX_AUTH` for password length, username rules, and inline errors. Same pipeline idea, different config surface.
...
| File uploads | `rx.upload` (see [File uploads](uploads.md)) |
