...
3. Configure [media serving](media.md) so `file_url` loads in the browser.

**Check:** Upload while logged in. Anonymous uploads will not see the expected `self.request.user`.

**Deep dive:** [File uploads](uploads.md)
...
- `RX_AUTO_MOUNT=True` so the SPA catch-all stays last
...
# SPA catch-all: automatic when RX_AUTO_MOUNT=True
```
...
**Deep dive:** [HTTP APIs alongside Reflex](api.md)
...
**Next up:** [Troubleshooting](../operations/troubleshooting.md) when something breaks in dev.
