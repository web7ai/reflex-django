...
1. Resolves a **bridge tier** for the handler's state class (`full`, `auth_only`, or `none`). Default project setting is `RX_EVENT_BRIDGE_MODE = "full"`  -  same as before tiered bridges.
...
4. Runs middleware for the tier  -  full `MIDDLEWARE` or `RX_AUTH_ONLY_MIDDLEWARE`.
...
Upload events always run at least `auth_only`. Override per class with `_rx_bridge` or project-wide with `RX_EVENT_BRIDGE_MODE`. Full recipes: [Scaling and performance](../operations/scaling.md).
...
```python
RX_AUTO_REDIRECT_FROM_MIDDLEWARE = False
```
...
```python
RX_EVENT_POST_FROM_PAYLOAD = True
```
...
1. Set `RX_EVENT_BRIDGE_MODE = "smart"` so plain `rx.State` skips middleware.
2. Override one class with `_rx_bridge = "none"` (underscore prefix  -  public attrs become Reflex state vars).
3. Skip heavy middleware via `RX_EVENT_MIDDLEWARE_SKIP`.
4. Use `RX_PERFORMANCE_PRESET = "lean"` for smaller WebSocket deltas.
5. Last resort: `RX_RUN_MIDDLEWARE_CHAIN = False`.
