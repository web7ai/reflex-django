...
A page can mix several states. Use `rx.State` for a filter bar and `AppState` for user-specific data. Use `_reflex_django_bridge` (underscore prefix) for per-class tier overrides  -  public class attrs become Reflex state vars. See [Scaling and performance](../operations/scaling.md).
