...
Use `AppState` when your handlers need `request.user`, the session, or Django middleware. It is a Reflex `State` subclass that fills `self.request` and related fields before each `@rx.event` runs.

Use plain `rx.State` for UI-only pages. Use `ModelState` for declarative CRUD. This page covers `AppState` in depth.
...
With `RX_EVENT_BRIDGE_MODE = "smart"`, plain `rx.State` handlers use tier `none` automatically  -  no middleware, no `self.request`. `AppState` and `ModelState` still get tier `full`.

If middleware returns a 3xx (for example login required), the bridge converts it to `rx.redirect(...)` unless you set `RX_AUTO_REDIRECT_FROM_MIDDLEWARE = False`.

See [WebSocket event pipeline](../internals/event_pipeline.md) for the full plumbing and [Scaling and performance](../operations/scaling.md) for tuning.
...
class FilterState(rx.State):
    query: str = ""
    # With RX_EVENT_BRIDGE_MODE = "smart", tier "none" is automatic.
...
A page can mix several states. Use `rx.State` for a filter bar and `AppState` for user-specific data. Use `_rx_bridge` (underscore prefix) for per-class tier overrides  -  public class attrs become Reflex state vars. See [Scaling and performance](../operations/scaling.md).
...
When `RX_AUTH_AUTO_SYNC = True` (default), snapshot fields refresh on every event for `AppState` subclasses.
...
**Next up:** [Database integration](database.md)