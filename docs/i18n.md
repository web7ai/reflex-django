...
`path("i18n/setlang/", ...)` in `urlpatterns` is enough — `/i18n` is auto-detected as a Django-owned prefix.
...
The deprecated `REFLEX_DJANGO_I18N_EVENT_BRIDGE` setting is a no-op — language activation runs through the full middleware chain when `LocaleMiddleware` is installed. To skip locale handling on events, remove `LocaleMiddleware` from `MIDDLEWARE` or add it to `REFLEX_DJANGO_EVENT_MIDDLEWARE_SKIP`:

```python
REFLEX_DJANGO_EVENT_MIDDLEWARE_SKIP = (
    *DEFAULT_EVENT_MIDDLEWARE_SKIP,  # from reflex_django.event_handler
...
- **[Custom middleware in events](django_middleware_to_reflex.md)** — the underlying middleware that activates the language; use `current_language()` or `request.LANGUAGE_CODE` in handlers.
