...
| Existing `/api/` and templates | `REFLEX_DJANGO_RX_CONFIG`; page imports in `urls.py` |
...
The streaming middleware must be **last**. It keeps Django admin streaming responses ASGI-safe. See [Async streaming middleware](async_streaming_middleware.md).
...
If you deploy with WSGI today (`gunicorn` sync workers), plan a move to an ASGI server (uvicorn, granian, hypercorn). See [Deployment](deployment.md).
...
Same origin, same cookies. Mobile clients can keep hitting `/api/` while browsers get the SPA at `/`. See [Routing](routing.md).
...
**404 on `/api/orders/`**
Ensure `path("api/", ...)` appears in `urlpatterns` before the SPA catch-all. Auto-detection should pick up `/api`. For unusual layouts, set `REFLEX_DJANGO_DJANGO_PREFIX`.
...
It does run by default. See [Custom middleware in events](django_middleware_to_reflex.md) if something specific is skipped.
