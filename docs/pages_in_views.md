...
**What you'll learn:** How to declare Reflex pages with `@page`, when Python registers their routes, and how Django-owned URLs stay separate from the SPA.

**When you need this:**

- You added a page but it does not show up in the browser.
- You are unsure whether a route belongs in `urls.py` or in a `@page` decorator.

---

In reflex-django, a Reflex page is a Python function that returns an `rx.Component`, living in a Django app's `views.py` next to your models. There is no per-page `urls.py` entry and no separate `frontend/` folder required.

Think of it like Django views, except Reflex owns client-side navigation for SPA routes.
...
**Deprecated:** with `REFLEX_DJANGO_AUTO_DISCOVER_PAGES=True` (default), reflex-django scans `INSTALLED_APPS` and imports `{app}.views` during compile preparation. You may see a deprecation warning. Prefer explicit imports.

| Setting | Effect |
|:---|:---|
| `REFLEX_DJANGO_AUTO_DISCOVER_PAGES = False` | Only `{app_name}.views` is imported (from `REFLEX_DJANGO_RX_CONFIG["app_name"]`). |
...
If your team prefers a separate `frontend/` package, point `REFLEX_DJANGO_PAGE_PACKAGES` at the modules you want. See [Project structure](project_structure.md).
...
Explicit `import shop.views` (or `REFLEX_DJANGO_PAGE_PACKAGES`) loads these modules at import/compile time.
...
Customize URLs, copy, and branding via `REFLEX_DJANGO_AUTH` in settings. See [Login and sessions](authentication.md).
...
Extend the list with `REFLEX_DJANGO_RESERVED_REFLEX_PREFIXES` if needed (uncommon).
...
Open **`http://localhost:3000/`** for Reflex pages (default two-port dev). Admin and API are on `:8000`. See [Local development](local_development.md).

For the URL dispatcher, SPA catch-all, and WebSocket routing details, see [Routing](routing.md).
...
**Next up:** [AppState bridge](state_management.md)