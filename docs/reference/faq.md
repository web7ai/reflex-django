...
Default: `run_reflex` runs Vite on `:3000` and the Reflex backend on `:8000`. Django admin/API are mounted **in-process** inside the Reflex backend. Vite proxies all backend paths there. Set `RXDJANGO_PROXY_SERVER` only when Django runs separately. See [Routing](../internals/routing.md) and [Local development](../getting-started/local_development.md).
...
Not by default. `REFLEX_DJANGO_AUTO_MOUNT=True` appends the catch-all at startup. Call `reflex_mount()` only for overrides. [The three knobs](../overview/concepts.md).

### Why is there no `rxconfig.py`?

v1.0 uses `REFLEX_DJANGO_RX_CONFIG` in `settings.py`. [Configuration](../getting-started/configuration.md).
