...
### `make_dispatcher`
Path-prefix ASGI transformer attached to `app.api_transformer` during dev. Routes Django URL prefixes to Django ASGI and Reflex reserved paths to Reflex. [Routing](../internals/routing.md).

### `django_outer` / `reflex_outer`
Historical routing modes from older releases. Current projects use mount-only: plain `get_asgi_application()` plus `reflex_mount()`. See [Routing](../internals/routing.md).
