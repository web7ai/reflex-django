...
For the short map, read [The three knobs](mental_model.md) first. You will meet `REFLEX_DJANGO_RX_CONFIG`, `AppState`, `DjangoEventBridge`, and `make_dispatcher`. No need to memorize them yet.
...
Vite proxies Django prefixes to that URL instead of the Reflex backend. See [Routing](routing.md).
...
**Next up:** [The three knobs](mental_model.md) or [Local development](local_development.md).
